...
static double *gbuf, *gmat, *pmat, *cmat;
...
//  MPI_Barrier(MPI_COMM_WORLD);
    ...
    rhf_form_gmatrix ( mints, pmat, gbuf );
    if ( xints ) {
        rhf_form_gmatrix ( xints, pmat, gbuf );
    }
//  MPI_Barrier(MPI_COMM_WORLD);
...
void
rhf_mpi_MullPopAnalysis ( FILE * outfile )
{
...
void
rhf_mpi_getDipoleHam ( const double *fld )
{
    int i, ir, jr, ij;
    double dm[3], qm[6];
