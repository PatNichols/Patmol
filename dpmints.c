...
calcPDPM ( double *vints, int nlst )
{
    ...
    double axp, c1, f12, bxp, c12, pxp, abi, s12, pa[3], p[3], pb[3];
    ...
    for ( ipr = 0; ipr < npr1; ++ipr ) {
        ...
        for ( jpr = 0; jpr < je; ++jpr ) {
            ...
            abi = 0.5 * abi;
            formMDD ( dx, abi, pa[0], pb[0], lv1, lv2 );
            formMDD ( dy, abi, pa[1], pb[1], lv1, lv2 );
            formMDD ( dz, abi, pa[2], pb[2], lv1, lv2 );
            for ( lpr = 0; lpr < npalf; ++lpr ) {
                ...
                pq2 = normSqr ( pq );
                txp = pxp + qxp;
                sr = 2.*  c12 * c34 * SRTERM * s12 * abi * cdi / sqrt ( txp );
                w = pxp * qxp / txp;
                t = w * pq2;
...
dpm_ints ( double *hmat )
{
    ...
    int cen1, off1, cen2, off2, nls1, nls2, nlst, asize, dsize;
    double *vints;
    const Shell *sh1;
    const Shell *sh2;
